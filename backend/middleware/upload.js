...
const folderFor = (fieldname) => {
  ...
  return map[fieldname] || "misc";
};

// Uploads a single in-memory file buffer to Vercel Blob, returns the public URL
async function uploadToBlob(file) {
  const uniqueSuffix = Date.now() + "-" + Math.round(Math.random() * 1e9);
  const ext = file.originalname.split(".").pop();
  const key = `${folderFor(file.fieldname)}/${file.fieldname}-${uniqueSuffix}.${ext}`;
  ...
  return blob.url; // store this URL in your DB
}
...
const uploadSingle = (fieldName) => (req, res, next) => {
  ...
  uploadMiddleware(req, res, async (err) => {
    ...
    try {
      if (req.file) {
        req.file.url = await uploadToBlob(req.file);
      }
      next();
    } catch (uploadErr) {
      return res
    ...
    try {
      if (req.files) {
        for (const file of req.files) {
          file.url = await uploadToBlob(file);
        }
      }
      next();
    } catch (uploadErr) {
